...
import 'package:todoapi/data/model/todo/ntodomodel.dart';
...
class _TodoHomeState extends State<TodoHome> {
                      ...
                      onPressed: () {
                        if (id != null) {
                          ...
                          var ntodoModel = NTodoModel(
                              ...
                              title: titleContrller.text.toString(),
                              completed: false);
  ...
  Widget build(BuildContext context) {
    mTodo = firestore.collection(COLLECTION_USER_KEY).doc(UID).collection(
        COLLECTION_todo_KEY);
    return Scaffold(
      appBar: AppBar(title: Text("MY TODO",style: TextStyle(color: Colors.white),),
            ...
            child: Row(
              ...
              children: [
                ...
                SizedBox(
                  ...
                  width: 50,
                  child: ClipOval(
                    child: Image.asset('assets/images/userplaceholder.png',fit: BoxFit.fill,),
                  ),
                ),
                SizedBox(
                  width: 2,
                ...
                Column(
                  ...
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Text(mUser?.name ?? 'Hello, User'
                      ,
                      style: TextStyle(
                          fontSize: 16, color: Colors.purpleAccent.shade100),
                    ),
                    Text(
                      mUser?.email ?? 'email@example.com',
                      style: TextStyle(
  ...
  void getUserid() async {
    ...
    if (UID != null) {
      DocumentSnapshot userData = await firestore.collection(COLLECTION_USER_KEY).doc(UID).get();
      mUser = UserModel.fromJson(userData.data() as Map<String, dynamic>);
    }
  ...
  Future <void> logout() async {
    ...
    pref.remove(SignIn.PREF_USER_ID);
